...
custom JSON class
"""
from base.models import Topic, CourseStructureEntry
...
class JsonHandler:
    ...
    def create_structures_from_json_data(course, data):
        ...
        for i in range(0, len(data)):
            ...
            CourseStructureEntry.objects.create(topic=topic, course=course, index=order)
            if 'children' in main_topic.keys():
                ...
                for j in range(0, len(children)):
                    ...
                    topic = Topic.objects.get(title=child_topic_name)
                    CourseStructureEntry.objects.create(topic=topic, course=course, index=order)
    ...
    def create_json_topics_structure(course):
        ...
        for topic in course.get_sorted_topic_list():
            ...
            if len(structure_index) == 1:
                ...
                if len(CourseStructureEntry.objects.filter(course=course, index__startswith=str(structure_index[0]))) > 1:
